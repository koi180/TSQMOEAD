...
%--------------------------------------------------------------------------
    ...
    if strcmp(method, 'MUD')
        if nargin < 4
            bias = 0.5;  % 默认中心聚集
        end  
        [W,N] = MUD_centered(N,M,bias);
    else
        [W,N] = feval(method,N,M);
    end
...
function [W,N] = MUD_centered(N,M,bias)
% MUD with central clustering for MOEA/D
    if nargin < 3
        bias = 0.5;  % 默认中心聚集
    end   
    X = GoodLatticePoint(N,M-1);
    if bias > 0
        c = 1 / (1 + bias);               
        for i = 1:size(X,2)
            x = X(:,i);
            X(:,i) = (x.^c) ./ (x.^c + (1-x).^c);
            X(:,i) = min(max(X(:,i), 1e-6), 1-1e-6);
        end
    end
    X = X.^(1./repmat(M-1:-1:1,N,1));
    X = max(X,1e-6);
    W = zeros(N,M);
